...
trait GameDef:
  ...
  case class Block(b1: Pos, b2: Pos):
    ...
    require((this.b1.row <= this.b2.row) && (this.b1.col <= this.b2.col), "Invalid block position: b1=" + b1 + ", b2=" + b2)
    ...
    def left = if this.isStanding then                  this.deltaCol(-2, -1)
               else if this.b1.row == this.b2.row then  this.deltaCol(-1, -2)
               else                                     this.deltaCol(-1, -1)

    /** The block obtained by moving right */
    def right = if this.isStanding then                 this.deltaCol(1, 2)
                else if this.b1.row == this.b2.row then this.deltaCol(2, 1)
                else                                    this.deltaCol(1, 1)

    /** The block obtained by moving up */
    def up = if this.isStanding then                    this.deltaRow(-2, -1)
             else if this.b1.row == this.b2.row then    this.deltaRow(-1, -1)
             else                                       this.deltaRow(-1, -2)

    /** The block obtained by moving down */
    def down = if this.isStanding then                  this.deltaRow(1, 2)
               else if this.b1.row == this.b2.row then  this.deltaRow(1, 1)
               else                                     this.deltaRow(2, 1)
    ...
    def neighbors: List[(Block, Move)] = List(
      (this.left, GameDef.this.Move.Left),
      (this.right, GameDef.this.Move.Right),
      (this.up, GameDef.this.Move.Up),
      (this.down, GameDef.this.Move.Down)
    )
     ...
     */
    def isLegal: Boolean = GameDef.this.terrain(b1) && GameDef.this.terrain(b2)
