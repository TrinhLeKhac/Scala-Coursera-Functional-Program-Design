...
trait Solver extends GameDef:
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    case (block, moves) #:: tail => {
      ...
      val newMoves = newNeighborsOnly(neighborsWithHistory(block, moves), newExplored)
   ...
   */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = from(
    (startBlock, List[Move]()) #:: LazyList[(Block, List[Move])](), Set()
  )
  ...
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = {
    pathsFromStart filter {
      case (block, _) => (block._1 == goal) && (block._2 == goal)
    }
  }
